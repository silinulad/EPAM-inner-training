...
/**
 * JoinGroup action processes user request to join specified Group
 */
public class JoinGroup implements Action {
		...
		if (InputValidator.validateId(idParameter)) {
			...
			if (group != null) {
				...
				if (!userGroups.contains(group)) {
					...
					request.setAttribute(SUCCESS_PARAM, GROUP_JOIN_SUCCESS);
				} else {
					request.setAttribute(ERROR_PARAM, GROUP_ALREADY_JOINED_ERROR);
				}
