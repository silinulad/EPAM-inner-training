...
/**
 * AddFriend command processes the user request to add an another user into the
 * friend list
 */
		...
		if (InputValidator.validateId(idParameter)) {
			...
			if (friend != null && user.getUserId() != friendId) {
				if (!friends.contains(friend)) {
					...
					userFriendsDao.addFriend(friend, user);
					request.setAttribute(SUCCESS_PARAM, USER_ADD_FRIEND_SUCCESS);
				} else {
					request.setAttribute(ERROR_PARAM, USER_IS_FRIEND_ERROR);
				}
